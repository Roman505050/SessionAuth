...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "purpose",
            postgresql.ENUM(
                "REGISTRATION", "CHANGE_PASSWORD", name="email_code_purpose"
            ),
            nullable=False,
        ),
...
def downgrade() -> None:
    ...
    # # Drop enums only after dropping tables that reference them
    op.execute("DROP TYPE IF EXISTS email_code_status CASCADE;")
    op.execute("DROP TYPE IF EXISTS email_code_purpose CASCADE;")
